...
volatile uint8_t waveform_type = 3;
...
const uint8_t sine_wave[256] = { /* Dữ liệu mẫu sin 8-bit */
		  ...
		  223,226,229,232,235,238,241,244,247,250,253,255,255,255,255,255,
		  ...
		  79,76,73,70,67,64,61,58,55,52,49,46,43,40,37,34,
		  31,28,25,22,19,16,13,10,7,4,1,0,0,0,0,0,
...
int main(void) {
    ...
    while (1) {
        ...
        uint16_t adcValue = 0xA103;
        sprintf(buffer, "ADC: %d\n", adcValue);
        send_data(buffer); // Gửi qua UART
...
void UART2_Init(void) {
    ...
    GPIOA->MODER |= (2 << (2 * 2)) | (2 << (3 * 2)); // PA2, PA3 chế độ AF
...
void parse_uart_data(void) {
    ...
    char msg[50];
    sprintf(msg, "Waveform: %d, Frequency: %d Hz\n", waveform_type, frequency);
    send_data(msg);
}
...
void send_data(char *msg) {
    while (*msg) {
        while (!(USART2->SR & USART_SR_TXE));  // Chờ cho buffer trống
        USART2->DR = *msg++;
    }
    while (!(USART2->SR & USART_SR_TC));  // Đợi gửi xong ký tự cuối cùng
}
